	...
	for (pass = passes; pass; pass = pass->next) {
		...
		for (loop = 0; loop < pass->passes; loop++) {
			...
			if (pass->mode == PASS_MODE_X) {
				fprintf(file, "M4 S%d\nG0 X%.7g Y%.7g\nG1 F%d\nG1 X%.7g\nG1 X%.7g\nM5 S0\n",
					base_spindle, img->orgx, img->orgy, base_feed, img->orgx+img->mmw, img->orgx);
			}
			else if (pass->mode == PASS_MODE_Y) {
				fprintf(file, "M4 S%d\nG0 X%.7g Y%.7g\nG1 F%d\nG1 Y%.7g\nG1 Y%.7g\nM5 S0\n",
					base_spindle, img->orgx, img->orgy, base_feed, img->orgy+img->mmh, img->orgy);
			}
			else if (pass->mode == PASS_MODE_AXIS) {
				fprintf(file, "M4 S%d\nG0 X%.7g Y%.7g\nG1 F%d\nG1 Y%.7g\nG1 Y%.7g\nG1 X%.7g\nG1 X%.7g\nM5 S0\n",
					base_spindle, img->orgx, img->orgy, base_feed,
					img->orgy+img->mmh, img->orgy,
					img->orgx+img->mmw, img->orgx);
			}
			else if (pass->mode == PASS_MODE_DIAG) {
				fprintf(file, "M4 S%d\nG0 X%.7g Y%.7g\nG1 F%d\nG1 X%.7g Y%.7g\nG1 X%.7g Y%.7g\nM5 S0\n",
					base_spindle, img->orgx, img->orgy, base_feed,
					img->orgx+img->mmw, img->orgy+img->mmh,
					img->orgx, img->orgy);
			}
				...
				fprintf(file, "M4 S%d\nG0 X%.7g Y%.7g\nG1 F%d\n"
					...
					"M5 S0\n",
					base_spindle, img->orgx, img->orgy, base_feed,
					img->orgy+img->mmh, img->orgx+img->mmw, img->orgy, img->orgx);
			}
				...
				for (y = 0; y < img->h; y++) {
					...
					for (x = 0; x < img->w; x++) {
						...
						if (!curr_spindle && (!x0 || ymoved || x - x0 > 20)) {
							fprintf(file, "G0 X%.7g", img->orgx+xr); // no lf here, at least one X will follow
							if (ymoved) {
								ymoved = 0;
								fprintf(file, " Y%.7g", img->orgy+yr); // no lf here, at least one X will follow
							}
							fprintf(file, "\nG1 "); // no lf here, at least one X will follow
						}
						else
							fprintf(file, "X%.7g S%d\n", img->orgx+xr, curr_spindle);
					...
					if (curr_spindle)
						fprintf(file, "X%.7g S%d\n",
							img->orgx+roundf(x * img->mmw / img->w * 1000.0) / 1000.0, curr_spindle);
